...
scan_qcawificfg80211() {
	...
	for vif in $vifs; do
		...
		local tifname
		config_get tifname "$vif" ifname

		local vifname
		#modified by xiaomi
		[ $ifidx -gt "$st_idx" ] && vifname="wl${radioidx}$ifidx" || vifname="wl${radioidx}"
		[ -n $tifname ] && vifname="${tifname}"
...
# it sets the default country ID to 156 (CN STA). This is because the desired
...
enable_qcawificfg80211() {
	...
	# miwifi : we do not change channel 36 to 40
	...
	config_get dcs_enable "$device" dcs_enable "0"
...
#TODO
		...
		config_get maxsta "$vif" maxsta 124
		"$device_if" "$ifname" maxsta "$maxsta"
...
#TODO
		...
		if [ "$mode" != "sta" ]; then
			if [ "$txbf" -eq 0 ]; then
				...
				iwpriv "$ifname" he_ulmumimo 0
			else
				iwpriv "$ifname" he_mubfer 1
		...
		config_get he_ulofdma "$vif" he_ulofdma 0
	...
	[ -z "${ssid}" ] && ssid="test"
		...
		ap-hk*)
			if [ -f /etc/rc.d/*qca-nss-ecm ]; then
				echo $nss_olcfg >/etc/wifi_nss_olcfg
				echo $nss_ol_num >/etc/wifi_nss_olnum
				echo "$(($olcfg_axa + $olcfg_axg))" > /etc/wifi_nss_hk_olnum
			else
				echo 0 >/etc/wifi_nss_olcfg
				echo $nss_ol_num >/etc/wifi_nss_olnum
				echo "$(($olcfg_axa + $olcfg_axg))" > /etc/wifi_nss_hk_olnum
			fi
			;;
